...
\begin{figure}[H]
  ...
  \caption{Príklad priebehu senzorových dát počas celého gesta \uv{NAPOMENUTIE -- červený}
    vrátane následnej sekvencie bodovania pre modrého zápasníka.}
...
(\uv{Gesto rozpoznané} v~čase 2{,}96\,s) sa hodnoty akcelerometra ustália
v~rozsahoch zodpovedajúcich šikmej polohe ruky nadol, čím sa aplikácia
prepne do režimu napomenutia červeného zápasníka. V~druhej fáze rozhodca
vykonal tri po sebe nasledujúce flickovacie gestá (\uv{Blue point}
v~časoch 4{,}43\,s, 5{,}07\,s a~5{,}72\,s), ktoré sú v~hornej časti grafu
...
je v~režime napomenutia zamknuté výhradne pre súpera napomenutého
zápasníka. V~tretej fáze (\uv{Hand down} v~čase 8{,}00\,s) sa hodnoty
akcelerometra ustália v~rozsahoch zodpovedajúcich neutrálnej polohe ruky,
...
znázorňujú očakávané rozsahy hodnôt pre jednotlivé osi v~cieľovej polohe
aktivačného gesta \uv{NAPOMENUTIE -- červený} a~v~neutrálnej polohe gesta
\uv{Hand Down}.

Charakteristickým znakom polohy pri aktivačnom geste sú stredne záporné
hodnoty na osi $a_x$ (ruka je šikmo nadol) a~výrazne kladné hodnoty na
osi $a_z$. Hraničné hodnoty:
...
\begin{figure}[H]
  ...
  \caption{Príklad priebehu senzorových dát počas celého gesta \uv{NAPOMENUTIE -- modrý}
    vrátane následnej sekvencie bodovania pre červeného zápasníka.}
  \label{fig:subgesture-napomenutie-modry}
\end{figure}

Na obrázku~\ref{fig:subgesture-napomenutie-modry} je zobrazený priebeh
gyroskopických a~akcelerometrických dát počas vykonania aktivačného gesta
\uv{NAPOMENUTIE -- modrý} a~následnej sekvencie bodovania. V~prvej fáze
(\uv{Gesto rozpoznané} v~čase 2{,}99\,s) sa hodnoty akcelerometra ustália
v~rozsahoch zodpovedajúcich polohe ruky za chrbtom -- os $a_y$ (oranžová)
dosahuje výrazne záporné hodnoty okolo $-9\,\text{m/s}^2$, čo je kľúčový
rozlišovací znak oproti červenému variantu, a~os $a_z$ (zelená) sa pohybuje
v~okolí nuly. Aplikácia sa tým prepne do režimu napomenutia modrého
zápasníka. V~druhej fáze rozhodca vykonal dve po sebe nasledujúce flickovacie
gestá (\uv{Red point} v~časoch 6{,}25\,s a~7{,}01\,s), ktoré sú v~hornej
časti grafu viditeľné ako výrazné záporné špičkové výkyvy na osi $g_x$
(modrá) prekračujúce prahovú hodnotu $\flckBlueThreshold\,\text{DPS}$.
Každý z~týchto výkyvov pridelil jeden bod červenému zápasníkovi, keďže
bodovanie je v~režime napomenutia zamknuté výhradne pre súpera napomenutého
zápasníka. V~tretej fáze (\uv{Hand down} v~čase 9{,}34\,s) sa hodnoty
akcelerometra ustália v~rozsahoch zodpovedajúcich neutrálnej polohe ruky,
čo signalizuje ukončenie procesu napomenutia modrého zápasníka.

Pre os $a_y$ (oranžová) sme zvolili väčší rozsah, pretože ruka rozhodcu
nemusí smerovať presne za chrbát -- pri reálnom použití môže dochádzať
k~jemnému odklonu od ideálnej polohy. Farebné pásma na grafe akcelerometra
znázorňujú očakávané rozsahy hodnôt pre jednotlivé osi v~cieľovej polohe
aktivačného gesta \uv{NAPOMENUTIE -- modrý} a~v~neutrálnej polohe gesta
\uv{Hand Down}.

Charakteristickým znakom polohy pri aktivačnom geste sú stredne záporné
hodnoty na osi $a_x$ (ruka za chrbtom) a~výrazne záporné hodnoty na osi
$a_y$, čo zodpovedá otočeniu zápästia za telom. Hraničné hodnoty:
...
Porovnanie oboch alternatív ukazuje, že kľúčovými
rozlišovacími faktormi sú osi $a_y$ a~$a_z$.
Pri červenom variante sa $a_y$ pohybuje okolo nuly
a~$a_z$ dosahuje výrazne kladné hodnoty, zatiaľ čo
pri modrom variante $a_y$ dosahuje výrazne záporné
hodnoty (okolo $-9$) a~$a_z$ sa pohybuje v~okolí
nuly.
