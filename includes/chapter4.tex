...
\begin{itemize}
  ...
  \item \textbf{Spätná väzba pre rozhodcu} -- aplikácia
        môže poskytovať rozhodcovi okamžité potvrdenie
...
gest rozhodcu priamo zo zápästia, nie všetky inteligentné
hodinky sú pre tento účel vhodné. Z~funkcionálnych
a~nefunkcionálnych požiadaviek vyplynuli nasledujúce
požiadavky na inteligentné hodinky:
...
Hodinky Amazfit T-Rex 3 \cite{trex3_specs}
poskytujú nasledovné parametre:
...
\begin{itemize}
  ...
  \item \textbf{Vibračný motor:}
        programovateľný cez Zepp~OS\Gls{api}, umožňuje
        nastavovanie režimov a~dĺžky vibrácií pre poskytovanie
        spätnej väzby používateľovi~\cite{zepp_sensor_vibrator}
...
Konkrétny model akcelerometra~\cite{zepp_sensor_acc}, jeho dynamický rozsah ani
frekvenciu vzorkovania Zepp~OS \Gls{api}
nezverejňuje a~výrobca tieto údaje v~oficiálnej dokumentácii
hodiniek Amazfit T-Rex~3 neuvádza. \Gls{api} poskytuje len
voľbu z~niekoľkých prednastavených režimov vzorkovania
prostredníctvom metódy \texttt{setFreqMode()}, no nie samotnú
frekvenciu ani rozsah merania. Pre potreby rozpoznávania
gest v~našej práci boli zabudované parametre senzora dostatočné.